...
字节级断言与 schema 校验有一个共同盲区：文件可以结构合法、断言全过，却在真实排版引擎
渲染之后明显是坏的。[**Render-Truth Bench**](bench/) 就建在这个缺口上——9 项任务，每项
由一条常规编辑指令与一个**字面执行陷阱**组成：照指令字面直接执行，产出的文件校验全绿、
渲染出来却是错的。

2×2 析因设计（两套系统 × 视觉反馈可用性），每单元重复 3 次，共 **108 次运行**，
全部产物经真实 Microsoft Word / Excel 渲染后判定：

| 配置 | DDR ↓ | SFR ↓ | DAR ↑ | DAR* ↑ |
|---|---|---|---|---|
| **OfficeBuddy** · 视觉反馈开 | **26%** | **15%** | **85%** | **81%** |
| OfficeBuddy · 视觉反馈关 | 33% | 22% | 78% | 71% |
| OfficeCLI · 截图可用 | 63% | 63% | 37% | 19% |
| OfficeCLI · 截图禁用 | 63% | 59% | 41% | 24% |

**DDR** 缺陷交付率——产物含目标缺陷的运行占比，由确定性检测器判定，不含任何模型判断。
**SFR** 静默失败率——文件是坏的、且从未告知用户。**DAR** 缺陷察觉率。
**DAR\*** 排除本项目已针对性加固的两项任务后的 DAR，该口径下的通过不能由固化默认值解释。

引用这些数字前请先读局限。任务集刻意瞄准本架构针对的那一个维度，它**不度量**能力覆盖
广度、速度与 token 成本——在那几项上对比是反过来的（见 [`bench/REPORT.md`](bench/REPORT.md)）。
每单元 n=3，数个百分点的差异不可区分。评测代码、每项任务都必须通过的三态标定，
以及含渲染证据的完整报告都在 [`bench/`](bench/)：
[`RESULTS.md`](bench/RESULTS.md) 与 [`visual_report.html`](bench/visual_report.html)。
